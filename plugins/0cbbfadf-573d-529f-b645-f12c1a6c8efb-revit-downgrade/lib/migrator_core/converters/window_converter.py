# -*- coding: utf-8 -*-
"""
migrator_core.converters.window_converter
=========================================
Converts Window records, resolving host walls and family symbols.
"""
...
class WindowConverter(BaseConverter):

    def can_convert(self, record, context):
        return getattr(record, "category_name", "") in ("Windows", "窗")
    ...
    def _resolve_symbol(self, doc, record):
        ...
        col = DB.FilteredElementCollector(doc).OfClass(DB.FamilySymbol).OfCategory(DB.BuiltInCategory.OST_Windows)
        first_sym = None
        for sym in col:
            if not first_sym:
                first_sym = sym
